# Unit 5: Future Trends and Research in Generative AI

## AIAT 124 - Generative Artificial Intelligence

Unit training hours: 14

## What This Unit Teaches

Where generative AI is heading: real-world applications, current research
directions, and generation beyond text and images — music, audio and voice
synthesis (WaveNet, Jukebox), advanced image models (StyleGAN, Stable
Diffusion, DALL-E), and AI-assisted code generation (OpenAI Codex, GitHub
Copilot).

## Prerequisites

- Units 1–4 completed

## Examples (do in order)

1. `examples/01_generative_ai_applications.ipynb` — Survey of generative AI
   applications across industries.
2. `examples/02_future_trends_research.ipynb` — Current research directions
   and open problems in generative AI.
3. `examples/03_music_generation.ipynb` — AI music generation.
4. `examples/04_experimenting_advanced_generative_models.ipynb` —
   Experimenting with advanced models: StyleGAN, Stable Diffusion, DALL-E.
5. `examples/05_audio_voice_synthesis_wavenet_jukebox.ipynb` — Audio and
   voice synthesis with WaveNet- and Jukebox-style approaches.
6. `examples/06_code_generation_openai_codex_copilot.ipynb` — Code generation
   with models like OpenAI Codex and GitHub Copilot.

## Exercise

- `exercises/01_generation_exercise.ipynb` — building a creative writing
  assistant.

## Quiz

- `../QUIZZES/quiz_05.md`

## Next

Finish the course with the project in `../PROJECTS/` and the final exam in
`../ASSESSMENTS/`.

Solutions and answer keys are released by your instructor.
