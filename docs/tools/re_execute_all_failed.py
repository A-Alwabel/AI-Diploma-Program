#!/usr/bin/env python3
"""
Re-execute all currently failed notebooks to get fresh status.
"""
...
def main():
    """Re-execute all failed notebooks."""
    # Get list of currently failed notebooks
    ...
    print(f"🔧 Re-executing {len(failed_notebooks)} failed notebooks\n")
    
    results = []
    with ProcessPoolExecutor(max_workers=4) as executor:
        future_to_nb = {executor.submit(execute_single_notebook, nb): nb for nb in failed_notebooks}
        ...
        for future in as_completed(future_to_nb):
            ...
            if completed % 10 == 0 or completed == len(failed_notebooks):
                percent = (completed / len(failed_notebooks)) * 100
                print(f"  Progress: {completed}/{len(failed_notebooks)} ({percent:.1f}%) - Passed: {passed}, Failed: {failed}", flush=True)
